...
const getIndexOfGalleryItem = (
  data: Map<number, any> | Array<any>,
  tim: number
) => {
  if (Array.isArray(data)) {
    return data
...
const getIndexOfFlatlistItem = (
  data: Map<number, any> | Array<any>,
  postId: number
) => {
  if (Array.isArray(data))
    return data.findIndex(item => item.postId === postId);
...
export default function MediaViewerScreen({
  ...
  const onPageChanged = useCallback(
    ({ viewableItems, changed }: ViewableItems) => {
      const viewableItem = viewableItems.find(
        viewtoken => viewtoken.isViewable
      );
  ...
  const onVideoItemChanged = useCallback(({ changed }: ViewableItems) => {
    ...
    const changedVideoItems = changed.filter(viewtoken =>
      videoFormat.includes(viewtoken.item?.fileExtension)
    );
      ...
      () => [
        {
          ...
          onViewableItemsChanged: onPageChanged
        },
        {
          viewabilityConfig: videoItemViewabilityConfig,
          onViewableItemsChanged: onVideoItemChanged
        }
      ],
      []
    );
