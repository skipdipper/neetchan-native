...
export default function CatalogSortByMenu() {
  ...
  const sortBy = (
    ...
    // Index bump order
    const array = catalog.map((item, index) => ({
      ...item,
      ...(item.index === undefined && { index })
    }));
    ...
    const sortFns = {
      ...
      get reply() {
        return sortByReplyCount();
      },
      get image() {
        return sortByImageCount();
      },
      get newest() {
        return sortByNewest();
      },
  ...
  return (
    <View style={styles.container}>
      ...
      <RadioListTile
        title="Reply count"
        value={Sort.REPLY_COUNT}
        groupValue={checked}
        onChanged={handleChange}
      />
      <RadioListTile
        title="Image count"
        value={Sort.IMAGE_COUNT}
        groupValue={checked}
        onChanged={handleChange}
      />
      <RadioListTile
