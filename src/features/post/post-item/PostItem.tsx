  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <PostDetails name={item.name} postID={item.postId} time={item.time} />
        ...
        {item?.comment && <Comment comment={item.comment} catalog={catalog} />}

        {/* Alternative to Wrapping Replies inside a View 
                with flex: 1, justifyContent: 'flex-end' */}
        <Spacer />

        {item?.postReplies && <Replies replies={item.postReplies} />}
      </View>
    </View>
  );
}
