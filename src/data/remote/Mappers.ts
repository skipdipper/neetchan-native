...
const thumbnailUrl = (board: string, tim: number) =>
  `https://i.4cdn.org/${board}/${tim}s.jpg`;
const fileUrl = (board: string, tim: number, ext: string) =>
  `https://i.4cdn.org/${board}/${tim}${ext}`;

export const attachmentFromDto = (
  ...
  return Object.freeze({
    ...
    get thumbnailUrl() {
      return thumbnailUrl(board, attachmentDto.tim);
    },
    get fileUrl() {
      return fileUrl(board, attachmentDto.tim, attachmentDto.ext);
    }
  } as const);
};
...
export const originalPostFromDto = (
...
export const catalogPostFromDto = (
  catalogPostDto: CatalogPostDto
): CatalogPost => {
  ...
  return Object.freeze(
    Object.entries(obj).reduce((post, [key, value]) => {
      return value === undefined ? { ...post } : { ...post, [key]: value };
    }, {} as CatalogPost)
  );
};

export const threadPostFromDto = (
  threadPostDto: OriginalPostDto | ReplyPostDto
...
export const boardFromDto = (boardDto: BoardDto): Board => {
  ...
  const optional = Object.entries({
    ...
    return value ? { ...setting, [key]: value } : { ...setting };
  }, {});
